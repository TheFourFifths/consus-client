...
describe("ItemController", () => {
    ...
    describe("removeItemFault", () => {
        let dispatcherSpy, removeItemFault, getAllItems;

        beforeEach(() => {
            dispatcherSpy = sinon.spy(Dispatcher, "handleAction");
            removeItemFault = sinon.stub(api, "removeItemFault");
            getAllItems = sinon.stub(api, "getAllItems");
        });

        it("Dispatches errors properly when something goes wrong", () => {
            removeItemFault.returns(
                new Promise((resolve, reject) => {
                    reject("Oh man, you screwed up");
                })
            );

            return ItemController.removeItemFault("iabcd").then(() => {
                assert.isTrue(dispatcherSpy.called);
                assert.strictEqual(dispatcherSpy.getCall(0).args[0], "ERROR");
                assert.strictEqual(dispatcherSpy.getCall(0).args[1].error, "Oh man, you screwed up");
                Dispatcher.handleAction("CLEAR_ERROR");
        ...
        it('Dispatches properly after clearing fault', () => {
            removeItemFault.returns(
                new Promise(resolve => {
                    resolve({
                        item: {}
            ...
            return ItemController.removeItemFault('abcd').then(() => {
                assert.isTrue(dispatcherSpy.called);
                assert.strictEqual(dispatcherSpy.getCall(0).args.length, 2);
                assert.strictEqual(dispatcherSpy.getCall(0).args[0], "ITEM_FOUND");
                assert.strictEqual(dispatcherSpy.getCall(1).args.length, 2);
